...
gmd({
  ...
  if (!isSuperUser) {
    ...
    try {
        ...
        const { data: commitData } = await axios.get("https://api.github.com/repos/mauricegift/SIGMA-MDXI/commits/main");
        const latestCommitHash = commitData.sha;

        const currentHash = await getCommitHash();

        if (latestCommitHash === currentHash) {
            return reply("✅ Your SIGMA-MDXI Bot is Already on the Latest Version!");
        }
        ...
        await reply(`🔄 Updating SIGMA-MDXI Bot...\n\n*Commit Details:*\n👤 Author: ${authorName} (${authorEmail})\n📅 Date: ${commitDate}\n💬 Message: ${commitMessage}`);

        const zipPath = path.join(__dirname, '..', 'SIGMA-MDXI-main.zip');
        const { data: zipData } = await axios.get("https://github.com/mauricegift/SIGMA-MDXI/archive/main.zip", { responseType: "arraybuffer" });
        ...
        const sourcePath = path.join(extractPath, 'SIGMA-MDXI-main');
